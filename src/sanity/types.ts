...
// Query: *[  _type == "study" &&  defined(serial) &&  isPublic != false &&  (serial == $sNum || string(serial) == $sStr)][0]{  // 현재 문서의 기준 값  "categoryId": category._ref,  "serial": serial,  // 직전(위) 최대 2개: serial이 더 작은 것들, 앞에서 4개  "prev": *[    _type == "study" &&    defined(serial) &&    serial < ^.serial &&    select(defined(^.categoryId) => category._ref == ^.categoryId, true)  ] | order(serial desc) [0...4]{    "slug": string(serial),    title,    "date": coalesce(_createdAt, _updatedAt)  },  // 직후(아래) 최대 2개: serial이 더 큰 것들, 앞에서 4개  "next": *[    _type == "study" &&    defined(serial) &&    serial > ^.serial &&    select(defined(^.categoryId) => category._ref == ^.categoryId, true)  ] | order(serial desc) [0...4]{    "slug": string(serial),    title,    "date": coalesce(_createdAt, _updatedAt)  }}
