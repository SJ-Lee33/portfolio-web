...
export type Project = {
  ...
  links?: Array<{
    label?: string
    url?: string
    icon?: 'github' | 'globe' | 'file' | 'play' | 'external'
    _key: string
  }>
  overviewDesc?: string
...
export type PROJECT_QUERYResult = {
  ...
  links: Array<{
    label: string | null
    url: string | null
    icon: 'external' | 'file' | 'github' | 'globe' | 'play' | null
  }> | null
  ...
  resultOutcomes: Array<string> | null
  resultMetrics: Array<{
    label?: string
    before?: number
    after?: number
...
// Query: *[  _type == "study" &&   !(_id in path("drafts.**")) &&  isPublic != false &&  defined(serial) ] | order(serial desc) {  "slug": string(serial),  title,  "skill": skill[],  "thumbnail": coalesce(thumbnail.asset->url, ""),  studyTypes,  "createdAt":_createdAt,  "updatedAt":_updatedAt,}
...
// Query: *[    _type == "studyCategory" &&    !(_id in path("drafts.**")) &&        // ⬅️ 루트에서 draft 제외    defined(slug) &&                 // ⬅️ slug 없는 문서 제외(안전망)    isPublic != false  ] | order(title asc) {    _id,    title,    slug,    skill[],    studyTypes,    summary,    "thumbnail": coalesce(thumbnail.asset->url, ""),    "recentFivePosts": *[      _type == "study" &&       !(_id in path("drafts.**")) &&      isPublic != false &&      references(^._id)    ] | order(_createdAt desc)[0...3]{      _id,      title,      "slug": string(serial),      "updatedAt":_updatedAt,      "thumbnail": coalesce(thumbnail.asset->url, ""),    }  }
...
// Query: *[_type=="studyCategory" && slug==$categorySlug][0]{  _id,  title,  slug,  summary,  "thumbnail": coalesce(thumbnail.asset->url, ""),  skill[],  // 총 개수  "totalCount": count(*[    _type=="study" && !(_id in path("drafts.**")) && references(^._id) && isPublic != false   ]),  // 페이지 슬라이스  "studyPosts": *[    _type=="study" && !(_id in path("drafts.**")) && references(^._id) && isPublic != false  ] | order(_createdAt desc){    _id,    title,    "slug": string(serial), // 라우팅 키: /study/[serial]    "updatedAt": _updatedAt,    "thumbnail": coalesce(thumbnail.asset->url,""),  }}
...
// Query: *[  _type == "study" &&  defined(serial) &&  // isPublic != false &&  (    serial == $sNum ||           // number 비교    string(serial) == $sStr      // string 비교  )][0]{  // "키 이름" : 표현식  // 따옴표 없으면 동일한 이름  "slug": string(serial),  isPublic,  title,  "categoryTitle": studyCategory->title,  "categorySlug": studyCategory->slug,  serial,  "skill": skill[],  "thumbnail": coalesce(thumbnail.asset->url, ""),  "body": body[],  "createdAt":_createdAt,  "updatedAt":_updatedAt,}
...
// Query: *[  _type == "study" &&  defined(serial) &&  isPublic != false &&  (serial == $sNum || string(serial) == $sStr)][0]{  // 현재 문서의 기준 값  "categoryId": category._ref,  "serial": serial,  // 직전(위) 최대 2개: serial이 더 작은 것들, 앞에서 4개  "prev": *[    _type == "study" &&    defined(serial) &&    serial < ^.serial &&    isPublic != false &&    select(defined(^.categoryId) => category._ref == ^.categoryId, true)  ] | order(serial desc) [0...4]{    "slug": string(serial),    title,    "date": coalesce(_createdAt, _updatedAt)  },  // 직후(아래) 최대 2개: serial이 더 큰 것들, 앞에서 4개  "next": *[    _type == "study" &&    defined(serial) &&    serial > ^.serial &&    isPublic != false &&    select(defined(^.categoryId) => category._ref == ^.categoryId, true)  ] | order(serial desc) [0...4]{    "slug": string(serial),    title,    "date": coalesce(_createdAt, _updatedAt)  }}
