...
// Query: *[    _type == "studyCategory" &&    !(_id in path("drafts.**")) &&        // ⬅️ 루트에서 draft 제외    defined(slug)                 // ⬅️ slug 없는 문서 제외(안전망)  ] | order(title asc) {    _id,    title,    slug,    skill[],    studyTypes,    summary,    "thumbnail": coalesce(thumbnail.asset->url, ""),    "updatedAt":_updatedAt,    "recentFivePosts": *[      _type == "study" &&       !(_id in path("drafts.**")) &&      references(^._id)    ] | order(_createdAt desc)[0...5]{      _id,      title,      "slug": string(serial),      "createdAt": _createdAt,      "thumbnail": coalesce(thumbnail.asset->url, ""),      summary    }  }
